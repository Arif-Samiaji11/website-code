...
def submitted_data():
    # Retrieve the submitted data from the database or any storage mechanism
    # For now, let's assume you have a list of submitted data
    submitted_data_list = [...]  # Replace with your actual data retrieval logic
    return render_template('submitted_data.html', submitted_data_list=submitted_data_list)
from flask import redirect, url_for

@app.route('/submit', methods=['POST'])
def submit_form():
    # Your form submission logic here
    # Assuming you have stored the submitted data in a list
    # Replace this with your actual data storage mechanism

    submitted_data_list.append({
    ...
    return redirect(url_for('submitted_data'))
...
# Assuming you have a truck cascade classifier file
...
def index():
    ...
    if request.method == "POST":
        ...
        # generating a unique name to save the image
        ...
        ccnt = 0
        # If no buses detected, check for cars
        if bcnt == 0:
            for (x, y, w, h) in cars:
        ...
        tcnt = 0
        # Cascade for trucks
        truck_cascade = cv2.CascadeClassifier(truck_cascade_src)
        trucks = truck_cascade.detectMultiScale(grey, 1.1, 1)
        ...
        result = f"{ccnt} cars, {bcnt} buses, and {tcnt} trucks found"
...
# Define a list to store submitted data
submitted_data_list = []
...
@app.route('/submitted_data')
def submitted_data():
    return render_template('submitted_data.html', submitted_data_list=submitted_data)

# Route to submit form
@app.route('/submit', methods=['POST'])
def submit_form():
    # Your form submission logic here
    # Assuming you have stored the submitted data in the list
    # Replace this with your actual data storage mechanism
